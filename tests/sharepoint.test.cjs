...
function setup(failAt) {
  ...
  }, SLITTER_SHAREPOINT: { siteId: 'site', schemaVerified: true, lists: { SlitterRecord: { id:'header', fields:{docNo:'Title',status:'Status'} }, KnifeUsage: {id:'knife', fields:{docNo:'Title',knifeNo:'Knife'}} } },
  fetch: async (url, options) => {
    calls.push({url,options});
    if (failAt === calls.length) throw new Error('network lost');
    const data = options.method === 'POST' ? {id:String(calls.length)} : {value: ['Title','Status','Knife'].map(name => ({name}))};
...
const groups = [{list:'SlitterRecord',rows:[{docNo:'20260910D-3A',status:'Pending'}]},{list:'KnifeUsage',rows:[{docNo:'20260910D-3A',knifeNo:'01'}]}];
...
test('all lists saved before Complete; token reused',async()=>{
 ...
 assert.equal(JSON.parse(s.calls.at(-1).options.body).Status,'Complete');
...
test('missing mapping blocks every write',async()=>{
 const s=setup(); delete s.context.SLITTER_SHAREPOINT.lists.KnifeUsage.fields.knifeNo;
